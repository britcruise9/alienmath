...
const MODE_SIMPLE_BALANCE = 3;
const MODE_COLOR_BALANCE = 4;
const MODE_NESTED_BALANCE = 5;
...
export default function GalacticGame() {
  ...
  const appStateRef = useRef({
    mode: MODE_HUB, // Skip boot for debug
    frame: 0,
    mouse: {x: 0, y: 0, down: false},
    debugSteps: [
      { id: 1, x: 150, y: 225, r: 20, label: "1" },
      { id: 2, x: 250, y: 225, r: 20, label: "2" },
      { id: 3, x: 350, y: 225, r: 20, label: "3" },
      { id: 4, x: 450, y: 225, r: 20, label: "4" }
    ],
    pattern: {
      sequence: [] as string[],
    ...
    nestedBalance: {
      ...
      leftMini: { ang: 0, tgtAng: 0, pos: -3 },
      rightMini: { ang: 0, tgtAng: 0, pos: 3 },
      blocks: [] as Block[],
      drag: null as Block | null,
      winT: 0
    }
  ...
  useEffect(() => {
    ...
    function handleInputDown() {
      ...
      else if (appState.mode === MODE_SIMPLE_BALANCE) handleBalanceDown();
      else if (appState.mode === MODE_COLOR_BALANCE) handleColorBalanceDown();
      else if (appState.mode === MODE_NESTED_BALANCE) handleNestedBalanceDown();
    }
    ...
    function handleInputUp() {
      if (appState.mode === MODE_SIMPLE_BALANCE) handleBalanceUp();
      else if (appState.mode === MODE_COLOR_BALANCE) handleColorBalanceUp();
      else if (appState.mode === MODE_NESTED_BALANCE) handleNestedBalanceUp();
    }
